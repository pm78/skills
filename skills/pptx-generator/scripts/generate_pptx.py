...
def _load_config(config_path: Path) -> tuple[Dict[str, Any], bool]:
    ...
    return config, "presentation" in config
...
def _export_snapshots(
    *,
    ...
    export_script = Path(__file__).resolve().with_name("export_slides.py")
    cmd = [
        sys.executable,
        str(export_script),
        "--pptx",
    ...
    *,
    ...
    for idx, slide in enumerate(slides, start=1):
        ...
        if layout == "two-column":
            # Split early if image suggests overflow/clipping.
            left = str(slide.get("left", "") or "")
            right = str(slide.get("right", "") or "")
...
def _run_qa_pipeline(
    *,
    ...
    for pass_idx in range(1, max(1, qa_passes) + 1):
        changes_in_last_pass = False
        generator = PPTXGenerator.from_dict(
            config,
            theme=theme,
        ...
        snapshots_ok = _export_snapshots(
    ...
    # If we ended the loop right after applying changes (pass limit reached),
    # regenerate once so the output PPTX matches the QA-adjusted config.
...
def main() -> None:
    ...
    parser.add_argument(
        "--qa-config-out",
        default=None,
        help="Optional path to write the QA-adjusted config JSON (default: <config>.qa.json when --qa)",
    )

    args = parser.parse_args()

    default_template = Path(__file__).resolve().parent.parent / "assets" / "worldline-template.pptx"
    ...
    if args.qa:
        ...
        qa_config_out = (
            ...
            else config_path.with_suffix(".qa.json")
        )
        _run_qa_pipeline(
