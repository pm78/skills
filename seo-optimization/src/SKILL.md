...
### Polylang Multilingual Automation (learned from production)

- **Polylang free has NO REST API** — the `polylang` namespace is not registered. All language management must go through custom Code Snippet REST endpoints.
- **`add_language()` via `admin_init` hook fails silently** — Polylang's wizard must be "completed" first. Workaround: create a custom REST endpoint (`register_rest_route`) that calls `PLL()->model->add_language($lang)` and trigger it via authenticated POST. This bypasses the wizard gate.
- **Language assignment functions** — Use `pll_set_post_language($post_id, 'fr')` and `pll_save_post_translations(['en' => $en_id, 'fr' => $fr_id])` to link translations. Same for terms: `pll_set_term_language()` + `pll_save_term_translations()`.
- **All existing content must be assigned to the default language** before creating translations. Loop all posts, pages, categories, and tags with `pll_get_post_language()` check → `pll_set_post_language()`.
- **Polylang options structure** — `get_option('polylang')` returns: `force_lang` (1=URL prefix), `hide_default` (1=hide /en/), `rewrite` (1=pretty), `default_lang`, `redirect_lang` (browser detect).
- **Language switcher** — Deploy via Code Snippet: `pll_the_languages(['raw' => 1])` returns array with `slug`, `name`, `url`, `current_lang`. Inject into header via JS `appendChild` on `.ts-header-inner` or equivalent nav container.
- **hreflang tags** — Polylang auto-injects `<link rel="alternate" hreflang="...">` once translations are linked. No manual hreflang needed.
- **Block editor pages have empty `content.raw`** — When translating, fetch `content.rendered` instead. Create translated pages using rendered HTML with text replaced.
- **`/wp/v2/plugins` CAN work with Application Passwords** — Contrary to some older WP versions, plugin install/activate via `POST /wp/v2/plugins {"slug":"polylang","status":"active"}` works on WP 6.8+ with proper admin Application Passwords. Always test before assuming 401.

### Dynamic OG Image Strategy (learned from production)

- **Create a helper function** (`ts_sitename_social_image()`) that checks `has_post_thumbnail()` on singular pages, returns the featured image URL if available, falls back to a generic 1200x630 branded card.
- **Hook into Yoast** via `wpseo_opengraph_image` and `wpseo_twitter_image` filters (priority 20).
- **Also output manual OG tags via `wp_head`** (priority 35) as a safety net — some Yoast configurations don't fire all OG hooks.
- **Image dimensions** — Always include `og:image:width` (1200), `og:image:height` (630), `og:image:alt` in the `wp_head` output.

### Featured Image Batch Generation Workflow

1. Define brand palette and style constraints (abstract geometric, no text, no faces).
2. Generate images via OpenAI `gpt-image-1` at 1536x1024, then resize to 1200x630 with Pillow.
3. Upload via multipart form: `curl -F "file=@path;type=image/png;filename=fn" -F "alt_text=..." /wp/v2/media`.
4. Set `featured_media` on each post: `POST /wp/v2/posts/{id} {"featured_media": media_id}`.
5. Verify OG image renders correctly on live article pages.
